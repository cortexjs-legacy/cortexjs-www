...
var createDB = module.exports = function(options) {
  ...
  couch.users.extend({
    updateGithub: function(username, githubUrl, callback) {
      var id = 'org.couchdb.user:' + username;
      this.fetch(id, function(err, doc) {
        if (err) return callback(err);
        doc.github = githubUrl;
        this.save(id, doc, callback);
      });
    },
    findByGithub: function(githubUrl, callback) {
      this.view('_auth/userByGithub', {
        auth: config.admin
      }).query().groupLevel(2).betweenKeys([githubUrl], [githubUrl, {}]).exec(callback);
    },
