...
router.get('/package/:name', function(req, res, next) {
  Packages.load(req.params.name, function(err, pkg) {
    ...
    if (req.isAuthenticated()) {
      taggie.getItemTags(req.user.name, pkg.name, function(err, tags) {
        if (err) return next(err);
        return render(tags);
      })
    } else {
...
router.post('/api/collections', function(req, res, next) {
  ...
  var user = req.user.name;
  taggie.getItemsByUnionTags(user, req.body, function(err, items) {
    res.send(items);
  })
});
...
router.post('/api/collections/addtag', function(req, res, next) {
  ...
  var user = req.user.name;
  taggie.tag(user, req.body.tag, req.body.item, function(err, items) {
    if (err) {
...
router.post('/api/collections/removetag', function(req, res, next) {
  ...
  var user = req.user.name;
  taggie.untag(user, req.body.tag, req.body.item, function(err, items) {
    if (err) {
      return res.send(500, err.message);
    }
    res.send(200);
